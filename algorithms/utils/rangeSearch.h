...
RangeSearch(PointRange &Query_Points,
  ...
  parlay::parallel_for(0, Query_Points.size(), [&](size_t i) {
    ...
    QueryParams QP_range(QP.beamSize, QP.beamSize, 0.0, G.size(), G.max_degree(), QP.early_stop, QP.early_stopping_radius, 
            QP.is_early_stop, false);
    ...
    for (indexType j = 0; j < beamElts.size(); j++) {
      if(beamElts[j].second <= QP_range.radius) {
    ...
    if(neighbors.size() < QP.beamSize){
      ...
      parlay::sequence<indexType> ans;
      for (auto r : in_range) {
        if(r.second <= QP_range.radius) ans.push_back(r.first);
      }
      all_neighbors[i] = ans;
      QueryStats.increment_visited(i, in_range.size());
      QueryStats.increment_dist(i, dist_cmps);
