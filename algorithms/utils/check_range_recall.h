...
void checkRangeRecall(
  ...
  all_rr = RangeSearch<Point, PointRange, indexType>(Query_Points, G, Base_Points, QueryStats, start_point, QP);
...
void range_search_wrapper(Graph<indexType> &G, PointRange &Base_Points,
  ...
  for(long b: beams){
    ...
    // QueryParams QP(rad, b, is_early_stopping, is_double_beam, es, esr);
    QueryParams QP(k, Q, cut, limit, dg, rad, es, esr, false, false);
    long k, long Q, double cut, long limit, long dg, long es, double esr, bool ies, bool idb
