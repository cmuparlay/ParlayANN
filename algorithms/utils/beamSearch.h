...
beam_search_impl_with_set(Point p, GT &G, PointRange &Points,
  ...
  for (auto q : starting_points){
    if (!has_been_seen.count(q) > 0) has_been_seen.insert(q);
  ...
  while (remain > 0 && num_visited < QP.limit) {
    ...
    //   break;
    // }
    if(QP.is_early_stop && early_stopping<Point, PointRange, indexType, distanceType>(p, frontier, unvisited_frontier, rad, QP.early_stop, QP.early_stopping_radius, num_visited)){
      break;
    }
    G[current.first].prefetch();
    // add to visited set
