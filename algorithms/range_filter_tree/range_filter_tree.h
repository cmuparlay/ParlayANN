...
/* Rewriting the above with templatized index better faster stronger etc. */

template <typename T, typename Point, template <typename, typename, typename> class RangeSpatialIndex = PrefilterIndex, class PR = PointRange<T, Point>, typename FilterType = float_t>
struct RangeFilterTreeIndex {
    using pid = std::pair<index_type, float>;

    // because the spatial index is responsible for the points, we do not store them here
    std::unique_ptr<RangeSpatialIndex<T, Point, PR>> spatial_index;

    bool has_children = false;
    std::pair<
        std::unique_ptr<RangeFilterTreeIndex<T, Point, RangeSpatialIndex, SubsetPointRange<T, Point>, FilterType>>, 
        std::unique_ptr<RangeFilterTreeIndex<T, Point, RangeSpatialIndex, SubsetPointRange<T, Point>, FilterType>>> children;

    std::pair<FilterType, FilterType> range; // min and max filter values
    std::pair<FilterType, FilterType> median; // median filter values, delineating the range between the two children

    int32_t cutoff = 1000;

    size_t n;

    RangeFilterTreeIndex(std::unique_ptr<PR> points, const parlay::sequence<FilterType>& filter_values, int32_t cutoff = 1000, bool recurse = true) :
        spatial_index(std::make_unique<RangeSpatialIndex<T, Point, PR>>(std::move(points), filter_values)),
        cutoff(cutoff) {
    ...
    RangeFilterTreeIndex(py::array_t<T> points, py::array_t<FilterType> filter_values, int32_t cutoff = 1000) {
        py::buffer_info points_buf = points.request();
        if (points_buf.ndim != 2) {
            throw std::runtime_error("points numpy array must be 2-dimensional");
        }
        ...
        *this = RangeFilterTreeIndex<T, Point, RangeSpatialIndex, PR, FilterType>(std::make_unique<PR>(point_range), filter_values_seq, cutoff, true);
    }
    ...
    void build_children() {
        ...
        // if we were being deliberate, we would split on the first occurence of the median value
        auto n1 = n / 2;
        auto n2 = n - n1;
        ...
        std::unique_ptr<SubsetPointRange<T, Point>> points1 = points->make_subset(parlay::map(parlay::make_slice(indices.begin(), indices.begin() + n1), [&](auto i) { return i; }));
        std::unique_ptr<SubsetPointRange<T, Point>> points2 = points->make_subset(parlay::map(parlay::make_slice(indices.begin() + n1, indices.end()), [&](auto i) { return i; }));

        auto filter_values1 = parlay::sequence<FilterType>(this->spatial_index->filter_values.begin(), this->spatial_index->filter_values.begin() + n1);
        auto filter_values2 = parlay::sequence<FilterType>(this->spatial_index->filter_values.begin() + n1, this->spatial_index->filter_values.end());

        std::unique_ptr<RangeFilterTreeIndex<T, Point, RangeSpatialIndex, SubsetPointRange<T, Point>, FilterType>> index1 = std::make_unique<RangeFilterTreeIndex<T, Point, RangeSpatialIndex, SubsetPointRange<T, Point>, FilterType>>(std::move(points1), filter_values1, this->cutoff, false);
        
        std::unique_ptr<RangeFilterTreeIndex<T, Point, RangeSpatialIndex, SubsetPointRange<T, Point>, FilterType>> index2 = std::make_unique<RangeFilterTreeIndex<T, Point, RangeSpatialIndex, SubsetPointRange<T, Point>, FilterType>>(std::move(points2), filter_values2, this->cutoff, false);
        ...
        this->median = std::make_pair(this->children.first->range.second, this->children.second->range.first);
    } 
    ...
    void build_children_recursive() {
        ...
        parlay::par_do([&] {
            this->children.first->build_children_recursive();
        }, [&] {
