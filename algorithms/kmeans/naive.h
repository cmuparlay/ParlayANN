...
struct NaiveKmeans : KmeansInterface<T, Point, index_type, CT, CenterPoint> {
  ...
  void cluster_middle(T* v, size_t n, size_t d, size_t ad, size_t k, CT* c,
                      size_t* asg, Distance& D, kmeans_bench& logger,
                      size_t max_iter, double epsilon,
                      bool suppress_logging = false) {

    if (!suppress_logging)
      std::cout << "running kmeans, ad: " << ad << std::endl;

    // because we copy points into buffers of size 2048, we forbid larger values
    // of d
    if (d > 2048) {
    ...
    CT* new_centers =
       new CT[k * ad];   // do calculations for compute center inside here
    ...
    while (iterations < max_iter) {
      ...
      parlay::parallel_for(0, n, [&](size_t p) {
        ...
        auto distances = parlay::delayed::map(
           rangk, [&](size_t r) { return D.distance(buf, c + r * ad, d); });
      ...
      this->compute_centers(v, n, d, ad, k, c, new_centers,
                            asg);   // Compute new centers
      ...
      max_diff =
         *parlay::max_element(deltas);   // get the max movement of any center

      parlay::parallel_for(0, k * ad,
                           [&](size_t i) {   // copy back over centers
                             c[i] = new_centers[i];
                           });
      ...
      float setup_time =
         t.next_time();   // setup_time counts msse calculation time
      if (!suppress_logging)
        logger.add_iteration(iterations, assignment_time, update_time, msse, 0,
                             0, deltas, setup_time);

      if (max_diff <= epsilon)
        break;   // if our centers barely moved, we have essentially converged,
                 // so end k-means early
    }
    delete[] new_centers;
  }
