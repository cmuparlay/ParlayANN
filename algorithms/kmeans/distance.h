...
struct Mips_Distance : public Distance {
  ...
  float distance(uint8_t* p, uint8_t* q, unsigned d) {
    ...
    for (unsigned i = 0; i < d;
         i++) {   // changing type of i to unsigned to avoid compiler warnings
  ...
  float distance(int8_t* p, int8_t* q, unsigned d) {
    int result = 0;
    for (unsigned i = 0; i < d;
         i++) {   // changed type of i to unsigned to avoid compiler warnings
      result += ((int32_t)q[i]) * ((int32_t)p[i]);
    }
    return -((float)result);
  }
...
// EuclideanDistanceFast prohibits mixed type distance calls (much easier to
// debug), but used the fast distance implementation
