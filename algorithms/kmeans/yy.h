...
struct Yinyang : KmeansInterface<T, Point, index_type, CT, CenterPoint> {
  ...
  struct group {
    ...
    parlay::sequence<index_type>
       center_ids;   // store the ids of all the centers belonging to this group
    float max_drift;

    group(index_type id)
        : id(id), center_ids(parlay::sequence<index_type>()), max_drift(-1) {}
  };
  ...
  void cluster_middle(T* v, size_t n, size_t d, size_t ad, size_t k, CT* c,
    ...
    parlay::parallel_for(0, n, [&](size_t i) {
      ...
      for (size_t j = 0; j < k; j++) {
        ...
        if (j != pts[i].best && new_lb < lbs[i][centers[j].group_id])
          lbs[i][centers[j].group_id] = new_lb;
      }
    });
      ...
      parlay::parallel_for(0, n, [&](size_t i) {
        ...
        for (size_t j = 0; j < t; j++) {
          ...
          for (size_t l = 0; l < groups[j].center_ids.size(); l++) {
            ...
            distance_calculations[i]++;   // increment distance calc counter for
                                          // this pt
